...
int bmc_mbox_enqueue(struct bmc_mbox_msg *msg)
{
	...
	switch (msg->command) {
		...
		case MBOX_C_GET_FLASH_INFO:
			prlog(PR_INFO, "GET_FLASH_INFO: size: 0x%08x, erase: 0x%08x\n",
					LPC_SIZE, server_state.erase_granule);
			if (server_state.api == 1) {
				bmc_put_u32(msg, 0, LPC_SIZE);
				bmc_put_u32(msg, 4, server_state.erase_granule);
			} else {
				bmc_put_u16(msg, 0, LPC_SIZE >> server_state.block_shift);
		...
		case MBOX_C_BMC_EVENT_ACK:
			 ...
			 * it about, in sever_state.attn_reg. The caveat is that
...
int mbox_server_reset(unsigned int version)
{
	...
	server_state.erase_granule = 0x1000; /* Thats the minmum size for !v1 */
...
int mbox_server_init(void)
{
	...
	/* setup server */
	server_state.block_shift = 12; /* TODO add tests to change the block_shift */
