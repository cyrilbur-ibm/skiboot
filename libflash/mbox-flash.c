		...
		uint64_t len)
{
	...
	if (mbox_flash->version == 1) {
		...
		uint64_t window_len = len + (window_pos & mbox_flash_mask(mbox_flash));

		msg_put_u16(msg, 0, window_pos >> mbox_flash->shift);
		msg_put_u16(msg, 2, ALIGN_UP(window_len, 1 << mbox_flash->shift) >> mbox_flash->shift);
	}
