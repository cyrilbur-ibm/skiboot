...
int ast_sf_open(uint8_t type, struct spi_flash_ctrl **ctrl)
{
	...
	*ctrl = NULL;
	...
	/*
	 * Going to have to bikeshed a nice way to know if we;re on a 2400
	 * or 2500, for now, we're on a 2500...
	 */
	if (ast2500_sf_open(type, ct))
		goto fail;
...
void ast_sf_close(struct spi_flash_ctrl *ctrl)
{
	...
	/* Additional cleanup */
	/* TODO restore this, add the CONF reg to struct ?
	 *if (ct->type == AST_SF_TYPE_PNOR) {
